...
namespace _2022_Test.NSTEK.Models
{
    public static class DelayLawModel
    {
        ...
        public static List<PointD> GetElementsPosition(ProbeModel probe)
        {
            ...
            for (uint i = 0; i < elemNum; i++)
            {
                ...
                PointD e = new PointD(i * pitch - midRange, 0);
                positions.Add(e);
            }
        ...
        public static double[] GetElementDelays(
            List<PointD> elementPositions, // elements' positions
            double velocity, // Test block's velocity, in m/s
                             //double trueDepth // Focal point's true depth, in mm
            PointD focalPoint // Focal point's position in mm
            )
        {
            ...
            {
                PointD p = new PointD(elementPositions[i].X - focalPoint.X, elementPositions[i].Y - focalPoint.Y);
            ...
            )
        {
            ...
            for (int angleIndex = 0; angleIndex < angles.Count; angleIndex++)
            {
                ...
                PointD focalPoint = new PointD(focalX, focalY);
