...
namespace Power_Modbus_RTU_SAMPLE
{
    ...
    public class Lucid_Control_DI4
    { 
        public SerialPort spCOM;
        private float iLimitSecond;

        LucidControlDI4 lucid;
        ValueDI1 v;
        IoReturn ret;

        public Lucid_Control_DI4(string sPort)
        {
            try
            {
                lucid = new LucidControlDI4(sPort);
        ...
        public bool GETDI(double ch)
        {
            try
            {
                    v = new ValueDI1();
                    //v.SetValue(true);
                    lucid.GetIo((byte)ch, v);
        ...
        public string GETIO_SELFTEST(double ch)
        {
            //byte[] rev = new byte[13];
            bool pan = true;
            int temp1, temp2;
