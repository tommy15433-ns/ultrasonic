...
namespace Power_Modbus_RTU_SAMPLE
{
    ...
    public class MT4YMOD_dcpower
    { 
        public SerialPort spCOM;
        public int ADDR;

        public MT4YMOD_dcpower(string sPortName, int addr)
        {
            try
            {
        ...
        public bool ReadData(byte type, int sAddr,out string rev_data)
        {
            ...
            try
            {
                ...
                if (iLen > 0)
                {
                    ...
                    if (type == 0x04) //측정값
                    {
                        if (rev[11] != (byte)(CRC16_MOD(rev, 11)))
                            pan = false;
                        if (rev[12] != (byte)(CRC16_MOD(rev, 11) >> 8))
                            pan = false;
                    }
                }
                else
            ...
            if (pan)
            {
                temp1 = rev[4] | rev[3] << 8;
                temp2 = rev[6] | rev[5] << 8;
                rev_data = temp1.ToString() + ":" + temp2.ToString() + ":";
            }
            else
            {
                rev_data = "";
        ...
        public void WriteData(byte type, int sAddr, int indata)
        {
            byte[] rev = new byte[13];
            bool pan = true;
            int temp1, temp2;
            ...
            try
            {
                ...
                data[2] = (byte)(sAddr >> 8); //주소상위
                data[3] = (byte)(sAddr); //주소하위
                data[4] = (byte)(indata >> 8); //데이터
                data[5] = (byte)(indata); // 데이터
                data[6] = (byte)(CRC16_MOD(data, 6));
                data[7] = (byte)(CRC16_MOD(data, 6) >> 8);
                spCOM.Write(data, 0, data.Length);
                Thread.Sleep(50);
        ...
        public void SET_VOLT(int volt)
        {
            WriteData(06, 2001, volt * 100);
        }

        public void SET_OVP(int volt)
        {
            WriteData(06, 2003, volt * 100);
        }


        public void SET_CURRENT(int CURRENT)
        {
            WriteData(06, 2002, CURRENT * 1000);
        }

        public void SET_OCP(int CURRENT)
        {
            WriteData(06, 2003, CURRENT * 1000);
        }

        public void ONOFF(string volt)
        {
            if (volt == "ON")
            {
                WriteData(06, 2016, 1); // dcpower off
            }
            else
            {
                WriteData(06, 2016, 0); // dcpower off
            }

        }


        public string Read(int sAddr)
        {
            string sData = "";
